...
public class ASTBuilder implements MListener {
    ...
    public void exitWhileStmt(MParser.WhileStmtContext ctx) {
        map.put(ctx, new WhileStmt(
                        (AbstractExpr) map.get(ctx.expression()),
                        (AbstractStmt) map.get(ctx.statement()),
    ...
    public void exitBinaryExpr(MParser.BinaryExprContext ctx) {
        ...
        switch (ctx.op.getType()) {
            ...
            case MParser.Mod:
                ...
                break;
            case MParser.Add:
                op = BinaryExpr.BinaryOp.ADD;
                break;
            case MParser.Sub:
                op = BinaryExpr.BinaryOp.SUB;
                break;
            case MParser.LShift:
                op = BinaryExpr.BinaryOp.LEFT_SHIFT;
                break;
            case MParser.RShift:
                op = BinaryExpr.BinaryOp.RIGHT_SHIFT;
                break;
            case MParser.EQ:
                op = BinaryExpr.BinaryOp.EQ;
                break;
            case MParser.NE:
                op = BinaryExpr.BinaryOp.NE;
                break;
            case MParser.LT:
                op = BinaryExpr.BinaryOp.LT;
                break;
            case MParser.GT:
                op = BinaryExpr.BinaryOp.GT;
                break;
            case MParser.LE:
                op = BinaryExpr.BinaryOp.LE;
                break;
            case MParser.GE:
                op = BinaryExpr.BinaryOp.GE;
                break;
            case MParser.BOr:
                op = BinaryExpr.BinaryOp.BITWISE_OR;
                break;
            case MParser.BAnd:
                op = BinaryExpr.BinaryOp.BITWISE_AND;
                break;
            case MParser.BXor:
                op = BinaryExpr.BinaryOp.XOR;
                break;
            case MParser.Or:
                op = BinaryExpr.BinaryOp.LOGICAL_OR;
                break;
            case MParser.And:
                op = BinaryExpr.BinaryOp.LOGICAL_AND;
                break;
            case MParser.Assign:
                op = BinaryExpr.BinaryOp.ASSIGN;
                break;
            default:
    ...
    public void exitNewExpr(MParser.NewExprContext ctx) {
        map.put(ctx, new NewExpr(
                        (AbstractNewObject) map.get(ctx.newObject()), new SourcePosition(ctx.newObject())));
    }
    ...
    public void exitPostfixIncDec(MParser.PostfixIncDecContext ctx) {
        ...
        switch (ctx.op.getType()) {
            case MParser.AddAdd:
                op = UnaryExpr.UnaryOp.POSTFIX_INC;
                break;
            case MParser.SubSub:
                op = UnaryExpr.UnaryOp.POSTFIX_DEC;
                break;
            default:
                throw new RuntimeException("Unknown UnaryOp");
        }
        map.put(
                ctx,
                new UnaryExpr(
                        op, (AbstractExpr) map.get(ctx.expression()), new SourcePosition(ctx.expression())));
    }
    ...
    public void exitUnaryExpr(MParser.UnaryExprContext ctx) {
        ...
        switch (ctx.op.getType()) {
            case MParser.AddAdd:
                op = UnaryExpr.UnaryOp.PREFIX_INC;
                break;
            case MParser.SubSub:
                op = UnaryExpr.UnaryOp.PREFIX_DEC;
                break;
            case MParser.Add:
                op = UnaryExpr.UnaryOp.POS;
                break;
            case MParser.Sub:
                op = UnaryExpr.UnaryOp.NEG;
                break;
            case MParser.Not:
                op = UnaryExpr.UnaryOp.LOGICAL_NOT;
                break;
            case MParser.BNot:
                op = UnaryExpr.UnaryOp.BITWISE_NOT;
                break;
            default:
                throw new RuntimeException("Unknown UnaryOp");
        }
    ...
    public void exitNewArray(MParser.NewArrayContext ctx) {
        ...
        ClassType newType =
                new ClassType(type.getBaseType(), ctx.brackets().size() + ctx.expression().size());
    ...
    public void exitConstant(MParser.ConstantContext ctx) {
        ...
        switch (type) {
            case MParser.NumConst:
                map.put(ctx, new NumConst(Integer.valueOf(s)));
                break;
            case MParser.NullConst:
